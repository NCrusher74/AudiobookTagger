...
/// returns the contents of the Podcast Keywords tag as a string
struct KeywordsTag {
    ...
    func returnKeywordsMetadata() throws -> String {
        if self.audiobookFile.format == .mp3 {
            ...
            return mp4File.metadata.metadataItemsFiltered(
                byIdentifier: AudiobookTag.keywords.mp4Tag).first?.stringValue ?? ""
        }; return ""
    }
