...
/// returns the contents of the Album tag as a string
struct BookTitleTag {
    ...
    func returnBookTitleMetadata() throws -> String {
        if self.audiobookFile.format == .mp3 {
            ...
            return mp4File.metadata.metadataItemsFiltered(
                byIdentifier: AudiobookTag.bookTitle.mp4Tag).first?.stringValue ?? ""
        }; return ""
    }
