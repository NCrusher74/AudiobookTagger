...
/// returns the contents of the Genre tag as a string, does not use any pre-defined genres
struct GenreTag {
    ...
    func returnGenreMetadata() throws -> String {
        if self.audiobookFile.format == .mp3 {
            ...
            return mp4File.metadata.metadataItemsFiltered(
                byIdentifier: AudiobookTag.genre.mp4Tag).first?.stringValue ?? ""
        }; return ""
    }
