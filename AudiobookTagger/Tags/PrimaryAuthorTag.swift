...
/// returns the contents of the AlbumArtist tag as a string
struct PrimaryAuthorTag {
    ...
    func returnPrimaryAuthorMetadata() throws -> String {
        if self.audiobookFile.format == .mp3 {
            ...
            return mp4File.metadata.metadataItemsFiltered(
                byIdentifier: AudiobookTag.primaryAuthor.mp4Tag).first?.stringValue ?? ""
        }; return ""
    }
